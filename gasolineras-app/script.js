    ...
    methods: {
        async searchGasStations() {
            ...
            try {
                ...
                this.gasStations = this.processGasStations(data);

                if (this.gasStations.length === 0) {
                    this.error = 'No hay gasolineras dentro del radio indicado.';
                }
        ...
        processGasStations(data) {
                ...
                stations = data.ListaEESSPrecio
                    .map(station => {
                        ...
                        return {
                            ...
                            empresa: station['Rótulo'] || 'Desconocida',
            ...
            if (this.selectedFuel) {
                stations = stations.filter(station => {
                    if (this.selectedFuel === 'Gasolina 95') {
                        return station.gasolina95;
                    }

                    if (this.selectedFuel === 'Gasolina 98') {
                        return station.gasolina98;
                    }

                    if (this.selectedFuel === 'Diésel') {
        ...
        getLocation() {
            ...
            navigator.geolocation.getCurrentPosition(
                ...
                error => {
                    this.error = 'No se pudo obtener ubicación: ' + error.message;
                    this.loading = false;
                }
            );
        }
